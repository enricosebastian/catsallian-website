...
//CREATING A SINGLE POST ON HOME PAGE
                ...
                infoHeaderDiv.append(posterLinkA);
                    ...
                    posterInfoDiv.append(posterLinkA);
                        ...
                        postLocationDiv.append(posterLinkA);
                            ...
                            locationLinkA.setAttribute('href','#date');
                            var timestamp = new Date(postdetails.datecreated);
                            locationLinkA.append(timestamp.toLocaleString());
    ...
    $('.submit-button').on('click',function() {
        var caption = validator.trim($('.postingbox-text').val());
        var image = $('.upload-button')[0].files[0];

        if( validator.isEmpty(caption) ) {
            alert('you\'re missing a caption, silly!');
        } else if( validator.isEmpty($('.upload-button').val()) ) {
            alert('you\'re missing an image, silly!');
        } else {
...
//EDIT POST ACCEPTED
    $(document).on('click','.editingbox-submit-button',function() {
        ...
        var postcaption = validator.trim($('.editingbox-text').val());

        if( validator.isEmpty(postcaption) ) {
            alert('you\'re missing your new caption, silly!');
        } else {
...
// CREATING COMMENTS
            ...
            commentboxDiv.append(commentVoteColumnDiv);
                ...
                commentColumnDiv.setAttribute('class','comment-column');
                    ...
                    commentHeaderDiv.append(posterLinkA);
                        ...
                        commentDateDiv.setAttribute('class','comment-date');
                        var timestamp = new Date(commentdetails.datecreated);
                        commentDateDiv.append(timestamp.toLocaleString());
                    commentHeaderDiv.append(commentDateDiv);
    ...
    $('.comment-submit-button').on('click',function() {
        var commentcaption = validator.trim($('.commentingbox-text').val());
        $('.commentingbox-text').val('');
        var postparent = $('.selected-postbox').attr('id');

        if( validator.isEmpty(commentcaption) ) {
            alert('you\'re missing a comment, silly!');
        } else {
...
//EDIT COMMENT IS ACCEPTED
    $(document).on('click','.comment-editingbox-submit-button',function() {
        ...
        var commentcaption = validator.trim( $('.editingbox-text').val() );

        if( validator.isEmpty(commentcaption) ) {
            alert('you\'re missing a new caption, silly!');
        } else {
